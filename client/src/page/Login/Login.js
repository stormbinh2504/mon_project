...
function Login() {
  ...
  const [userqt, setUsertq] = useState("admin");
  const [passqt, setPassqt] = useState("admin");
  ...
  useEffect(() => {
    setfilterNVPHC(
      nguoidungId.filter((country) =>
        country.nhomQuyenId.tenNhomQuyen
          .toLowerCase()
          .includes(searchNVPHC.toLowerCase())
      )
    );
