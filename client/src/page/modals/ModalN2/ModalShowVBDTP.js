...
export default function ModalShowVBDTP(props) {
  ...
  console.log("datatbp", datatbp)
  // const [datattbpn, setdatattbpn] = useState(datattbp); //tên trưởng viên bộ phận
  ...
  useEffect(() => {
    setfilterNVTBP(
      nhanvienId.filter((country) =>
        (country.boPhanId._id == datatbp) && country.chucDanhId.tenChucDanh.toLowerCase().includes("Nhân viên".toLowerCase())
      )
    );
  }, [nhanvienId, datatbp]);
