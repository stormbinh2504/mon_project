...
function PaginationBCTKTP(props) {
  ...
  const [ngaykt, setngaykt] = useState(new Date("07/07/2021"));
  ...
  useEffect(() => {
    setfilterNgayden(
      congvieclanhdaoId.filter(
        // (country) => CheckDay(startDay, endDay, country.ngayDen.split('T')[0])
        (country) =>
          CheckDay(
            startDay,
            endDay,
            Moment(country.thoiHanXuLi).format().split("T")[0]
          )
      )
    );
  }, [congvieclanhdaoId, startDay, endDay]);
  ...
  useEffect(() => {
    setfilterTrangthai(
      congvieclanhdaoId.filter((country) =>
        country.trangThaiTruongPhong
          .toLowerCase()
          .includes(searchTrangthai.toLowerCase())
      )
    );
  }, [searchTrangthai, congvieclanhdaoId]);

  console.log("setfilterTrangthai", filterTrangthai);
  console.log("searchTrangthai", searchTrangthai);

  useEffect(() => {
    setfilterNhanvien(
      congvieclanhdaoId.filter((country) =>
        country.nhanVienId.tenNhanVien
          .toLowerCase()
          .includes(searchNhanvien.toLowerCase())
      )
    );
  }, [searchNhanvien, congvieclanhdaoId]);

  useEffect(() => {
    setfilterBophan(
      congvieclanhdaoId.filter((country) =>
        country.boPhanId.tenBoPhan
          .toLowerCase()
          .includes(searchBophan.toLowerCase())
      )
    );
  }, [searchBophan, congvieclanhdaoId]);
  ...
  for (
    let i = 1;
    i <= Math.ceil(congvieclanhdaoId.length / itemsPerPage);
    i++
  ) {
    pages.push(i);
  }
  ...
  const currentItems = congvieclanhdaoId.slice(
    indexOfFirstItem,
    indexOfLastItem
  ); //
  ...
  const currentNhanvien = filterNhanvien.slice(
    indexOfFirstItem,
    indexOfLastItem
  ); //
      ...
      <>
        {currentItems.map((item, index) => (
          <tr>
            ...
            <td>
              <Link>{item.vanBanDenId.taiLieu}</Link>
            </td>
          </tr>
        ))}
      </>
      ...
      <div className="content-func col-right">
        <div className="header-content ">
          ...
          <div className="export-csv">
            <ExportReactCSV
              csvData={congvieclanhdaoId}
              fileName="congvieclanhdao.xls"
            />
          </div>
        </div>
          ...
          <Table style={{ width: "100%" }} striped bordered hover>
            <thead>
              <tr>
                ...
                <th className="width-120px">Ngày đến</th>
                {/* <th className="width-180px">Cơ quan phát hành</th> */}
                <th className="width-180px">Bộ phận</th>
                <th className="width-180px">Trưởng phòng xử lí</th>
                {/* <th>Loại văn bản</th> */}
                <th>Trạng thái</th>
                <th>Đính kèm</th>
              </tr>
              ...
              {(() => {
                if (
                  ...
                  if (searchTrangthai != "") {
                    ...
                    return renderData(currentBophan);
                  } else if (searchNhanvien != "") {
                    return renderData(currentNhanvien);
...
export default PaginationBCTKTP;
