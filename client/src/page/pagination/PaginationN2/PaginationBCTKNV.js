...
function PaginationBCTKNV(props) {
  ...
  const [ngaykt, setngaykt] = useState(new Date("07/08/2021"));
  ...
  useEffect(() => {
    setfilterNgayden(
      congvietruongphongId.filter(
        // (country) => CheckDay(startDay, endDay, country.ngayDen.split('T')[0])
        (country) =>
          CheckDay(
            startDay,
            endDay,
            Moment(country.thoiHan).format().split("T")[0]
          )
      )
    );
  }, [congvietruongphongId, startDay, endDay]);
  ...
  for (
    let i = 1;
    i <= Math.ceil(congvietruongphongId.length / itemsPerPage);
    i++
  ) {
    pages.push(i);
  }
  ...
  const currentItems = congvietruongphongId.slice(
    indexOfFirstItem,
    indexOfLastItem
  ); //
      ...
      <div className="content-func col-right">
        <div className="header-content ">
          ...
          <div className="export-csv">
            <ExportReactCSV
              csvData={congvietruongphongId}
              fileName="congviectruongphong.xls"
            />
          </div>
        </div>
